    				...
    				<div>
    					...
    					<div class="col-lg-8 offset-lg-2 replies">
    						...
    						<div class="form-group ty-compact-list">
			                    ...
			                    <?php endif; ?>
				                <span class="float-right mr-4"><small><i>Crée le:
									<?php echo date('M d, Y h:i A',strtotime($rep['date_created'])) ?>
								</i></small></span>

			    				<p class="mb-0"><large><b><?php echo $rep['name'] ?></b></large>
									<span class="text-primary"><small class="mb-0"><i><?php echo $rep['username'] ?></i></small></span></p>
...
<script>
    ...
    function delete_topic($id){
        ...
        $.ajax({
            ...
            success:function(resp){
                if(resp==1){
                    alert_toast("Les données ont été supprimé avec succes",'success')
                    setTimeout(function(){
                        location.reload()
                    },1500)
