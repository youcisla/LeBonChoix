				...
				<div class="card">
					<div class="card-header">
						...
						<span class="">
							...
							<button class="btn btn-dark btn-block btn-sm col-sm-2 float-right" type="button" id="new_topic">
					<i class="fa fa-plus"></i> Creer une nouvelle discussion</button>
				</span>
					</div>
					<div class="card-body">
						<ul class="w-100 list-group" id="topic-list">
							...
							<li class="list-group-item mb-4">
									...
									<?php if($_SESSION['login_id'] == $row['user_id'] || $_SESSION['login_type'] == 1): ?>
					                    <div class="dropleft float-right mr-4">
					                      ...
					                      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
					                        <a class="dropdown-item edit_topic" data-id="<?php echo $row['id'] ?>" href="javascript:void(0)">Modifier</a>
					                        <a class="dropdown-item delete_topic" data-id="<?php echo $row['id'] ?>" href="javascript:void(0)">Supprimer</a>
					                      </div>
					                    </div>
				                    <?php endif; ?>
				                    <span class="float-right mr-4"><small>Crée le: <?php echo date('M d, Y h:i A',strtotime($row['date_created'])) ?></small></span>
								...
								<p class="row justify-content-left"><span class="badge badge-primary text-white">Posté par: <?php echo $row['name'] ?></span></p>
								<hr>
								
								<!-- <span class="float-left badge badge-secondary text-white"><?php //echo number_format($view) ?> view/s</span> -->
								<span class="float-left badge badge-dark text-white ml-2"><i class="fa fa-comments"></i>
									<?php echo number_format($comments) ?> Commentaire/s <?php echo $replies > 0 ? " and ".number_format($replies).' replies':'' ?>
								</span>
								...
								?>
								<span class="badge badge-dark text-white ml-2"><?php echo $tags[$cat] ?></span>
							<?php endforeach; ?>
								</span>
							</li>
						<?php endwhile; ?>
...
<script>
	...
	$('#new_topic').click(function(){
		uni_modal("Creer","manage_topic.php",'mid-large')
	})
	
	$('.edit_topic').click(function(){
		uni_modal("Modifier","manage_topic.php?id="+$(this).attr('data-id'),'mid-large')
	})
	$('.delete_topic').click(function(){
		_conf("Confirmer la suppression?","delete_topic",[$(this).attr('data-id')],'mid-large')
	})
	...
	function delete_topic($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Les données ont été supprimées",'success')
					setTimeout(function(){
						location.reload()
					},1500)
